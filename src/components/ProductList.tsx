...
const ProductList: React.FC<ProductListProps> = ({ onLogout }) => {
  const { token } = useAuth(); // Removed unused 'logout' since it's handled by prop
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (err.response?.status === 401 || err.response?.status === 403) {
        ...
        onLogout(); // Now correctly uses the prop
      } else {
  ...
  return (
    <div className="bg-white shadow-md rounded-lg overflow-hidden p-4">
      ...
      {loading ? <div className="overflow-x-auto">Loading...</div> : (
